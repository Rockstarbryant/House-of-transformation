...
const hasManageUsersPermission = (user) => {
  ...
  return user.role.permissions && 
         user.role.permissions.includes('manage:users');
};
 ...
 * Returns limited data for non-privileged users
 */
const sanitizeUserData = (user, requester) => {
  // If requester has manage:users or is admin, return full data
  if (hasManageUsersPermission(requester)) {
    return user;
  }
  
  // Otherwise, return limited public profile
  return {
    _id: user._id,
    name: user.name,
    avatar: user.avatar,
    role: user.role ? {
      _id: user.role._id,
      name: user.role.name
      // Don't expose permissions to regular users
    } : null
    // Don't expose: email, phone, location, bio, etc.
  };
};
...
// @route   GET /api/users
// @access  Private (requires manage:users permission)
exports.getAllUsers = asyncHandler(async (req, res) => {
  // ✅ CRITICAL: Verify permission server-side
  if (!req.user || !hasManageUsersPermission(req.user)) {
...
// @access  Private (any authenticated user can see their own profile)
...
exports.updateUserRole = asyncHandler(async (req, res) => {
  ...
  // ✅ CRITICAL: Verify admin permission
  if (!isAdmin(req.user)) {
    return res.status(403).json({ 
      success: false, 
...
// ============================================
// USER DELETION & BANNING
// ============================================

// @desc    Delete user (Admin only) - Hard delete from MongoDB and Supabase
...
// @access  Public (needs to be public for signup/login checks)
...
exports.deleteSelfAccount = asyncHandler(async (req, res) => {
  ...
  try {
    // Check if user signed up with OAuth (Google, Facebook, etc.)
